...
namespace {
    ...
    TEST(TCPTimestampOptionTest, GetSetTimestampPointer) {
        ...
        EXPECT_EQ(expected, ntohl(ts_option.get_wifu_tcp_timestamp()->timestamp_value_));

        ts_option.set_timestamp();
        expected = Utils::get_current_time_microseconds_32();
        EXPECT_NEAR(expected, ts_option.get_timestamp(), 100);
        EXPECT_NEAR(expected, ntohl(ts_option.get_wifu_tcp_timestamp()->timestamp_value_), 100);
    }
    ...
    TEST(TCPTimestampOptionTest, GetSetEchoPointer) {
        ...
        EXPECT_EQ(expected, ntohl(ts_option.get_wifu_tcp_timestamp()->timestamp_echo_reply_));
    }
}

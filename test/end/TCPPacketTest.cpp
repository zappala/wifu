...
namespace {
	...
	TEST(TCPPacket, EmptyConstructor) {
		TCPPacket p;
	...
	TEST(TCPPacket, SequenceNumberTest) {
	...
	TEST(TCPPacket, TCPHeaderLengthBytesTest) {
		TCPPacket p;
		ASSERT_EQ(sizeof (struct tcphdr), p.get_tcp_header_length_bytes());
	}

	TEST(TCPPacket, TCPHeaderLengthTest) {
		TCPPacket p;
		ASSERT_EQ(sizeof (struct tcphdr) / 4, p.get_tcp_header_length());
	}
	...
	TEST(TCPPacket, ReceiveWindowTest) {
	...
	TEST(TCPPacket, UrgentPointerTest) {
		RandomNumberSet<u_int16_t> random;
		TCPPacket p;

    ...
    SUITE(NetworkInterface) {
        ...
        TEST(NetworkInterface) {
            ...
            for (int i = 0; i < num_packets; i++) {
                ...
                CHECK_EQUAL(data.c_str(), (const char*) fnm.p->get_data());
                CHECK_EQUAL(packet_size, fnm.p->get_data_length_bytes());
                CHECK(*dest == *(fnm.p->get_dest_address_port()));
                CHECK(*source == *(fnm.p->get_source_address_port()));
            }
